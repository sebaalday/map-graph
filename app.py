...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 24 08:30:22 2019
...
df = pd.read_csv(os.path.join(APP_PATH, os.path.join("data", "pozos_coord.csv")))
df_level = pd.read_csv(
    os.path.join(APP_PATH, os.path.join("data", "niveles_acl.csv"))
)

# Cambio coordenadas
df['Latitude'], df['Longitude'] = utm.to_latlon(df['E'], df['N'], 19, 'J')
df_level['Fecha'] = pd.to_datetime(df_level['Fecha'])

# Assign color to legend
colormap = {}
for ind, tipo in enumerate(df["Tipo"].unique().tolist()):
                        ...
                        html.Div(
                            ...
                            children=[
                                ...
                                dcc.Dropdown(
                                    ...
                                    options=[
                                        {"label": i, "value": i}
                                        for i in df["Operador"].unique().tolist()
                                    ],
                                    multi=True,
                                    value=[
                                        df["Operador"].unique().tolist()[0],
                                        df["Operador"].unique().tolist()[1],
                                    ],
                                ),
                                dcc.Dropdown(
                                        id="well-select",
...
#                ),
                html.Div(
                    ...
                    id="well-level-container",
                    className="six columns",
                    children=[
                        build_graph_title("Well Level"),
                        dcc.Graph(id="level-fig"),
                    ],
                ),
            ],
        ),
...
def update_well_map(
        op_select, w_select, mapbox_view #     tern_selected_data, bar_selected_data, bar_click_data,
):
    dff = df[df["Operador"].isin(op_select)]
    tipos = dff["Tipo"].unique().tolist()
...
#    if prop_id == "ternary-map":
    for tipo in tipos:
        if w_select is None:
            ...
            processed_data[tipo] = df[df["Pozo"].isin(w_select)]
...
def update_figure(puntos_muestreo_selected,s_date,e_date):
    ...
    for i,val_selected in enumerate(puntos_muestreo_selected):
        df_filtered = df_level[(df_level['Pozo'] == val_selected)].copy()
